...
CORE_MIN_QUALITY="${SILVER_CORE_REFRESH_MIN_QUALITY:-25}"
...
GANGWON_CORE_MIN_QUALITY="${SILVER_GANGWON_CORE_REFRESH_MIN_QUALITY:-30}"
...
if ! [[ "$CORE_MIN_QUALITY" =~ ^[0-9]+$ ]] || [ "$CORE_MIN_QUALITY" -gt 100 ]; then
  echo "SILVER_CORE_REFRESH_MIN_QUALITY must be an integer between 0 and 100: $CORE_MIN_QUALITY" >&2
  exit 1
fi
...
if ! [[ "$GANGWON_CORE_MIN_QUALITY" =~ ^[0-9]+$ ]] || [ "$GANGWON_CORE_MIN_QUALITY" -gt 100 ]; then
  echo "SILVER_GANGWON_CORE_REFRESH_MIN_QUALITY must be an integer between 0 and 100: $GANGWON_CORE_MIN_QUALITY" >&2
  exit 1
fi
...
$CORE_REFRESH_CRON cd $APP_DIR/deploy && SILVER_REFRESH_MODE=core SILVER_REFRESH_REGIONS="$REGIONS" SILVER_REFRESH_LIMIT=$CORE_LIMIT SILVER_REFRESH_MIN_QUALITY=$CORE_MIN_QUALITY ./scripts/refresh-data.sh >> $LOG_DIR/refresh-core.log 2>&1
...
$GANGWON_REFRESH_CRON cd $APP_DIR/deploy && SILVER_REFRESH_MODE=core SILVER_REFRESH_REGIONS="$GANGWON_REGIONS" SILVER_REFRESH_LIMIT=$GANGWON_CORE_LIMIT SILVER_REFRESH_MIN_QUALITY=$GANGWON_CORE_MIN_QUALITY ./scripts/refresh-data.sh >> $LOG_DIR/refresh-gangwon.log 2>&1
