...
- 공지사항/알림판: 유족이 방문자에게 전할 안내, 추모식 일정, 기록 업데이트를 상단에 표시
- 방명록: 방문자 작성, 승인 대기, 공개 글 표시, 작성 브라우저에서 본인 글 수정/삭제
- 유족 편집기: 기본 정보 수정, 대표 사진 미리보기/업로드, 공개 범위 변경, 공지사항 관리, 방명록 승인/숨김/고정
...
- 버전 상세: 변경 전/후 스냅샷을 저장해 나무위키형 diff/복원 기능으로 확장할 기반 마련
...
- 운영 백오피스: `#/admin`에서 공공데이터 수집 현황, 추모관, 가족 권한, 방명록 대기, 제휴 문의 요약 확인, 문의 상태/담당자/다음 연락일/운영 메모 관리, 가족 권한 역할/상태 변경
...
POST /api/memory/memorials/kim-youngsu/announcements
PATCH /api/memory/announcements/{id}
DELETE /api/memory/announcements/{id}
...
공지사항은 방문자 화면의 상단 알림판에 표시됩니다. 유족 편집기에서 `공지`, `일정`, `업데이트` 유형으로 추가·수정·삭제할 수 있고, 중요한 안내는 상단 고정할 수 있습니다. 모든 공지 변경은 편집 기록에 남습니다.
...
백업 다운로드 파일에는 유족 코드와 초대 토큰 원문을 포함하지 않습니다. 권한 코드는 별도 보관해야 합니다. 백업에는 공지사항, 가족 권한 요약, 버전 상세도 포함합니다. 백업 가져오기는 먼저 JSON 형식을 확인하고, 현재 단계에서는 운영 DB를 덮어쓰지 않고 이 브라우저에 임시 복구합니다.

유족 협업 저장소는 현재 PostgreSQL을 유지합니다. 생애 기록은 개인정보와 삭제/비공개 정책이 중요하므로 Git 저장소처럼 외부 파일 이력에 남기는 방식보다 DB 안에서 권한과 함께 관리하는 편이 안전합니다. `contentRevisions`는 변경 전/후 스냅샷을 `jsonb`로 보관해, 이후 diff 보기와 특정 버전 복원 기능으로 확장합니다.
...
운영 백오피스는 `http://127.0.0.1:5175/#/admin` 또는 운영 경로 `/memory/#/admin`에서 열 수 있습니다. 로컬 기본 관리자 토큰은 `local-admin`이고, 운영에서는 반드시 `SILVER_ADMIN_TOKEN`을 별도로 설정합니다. 백오피스는 공공데이터 타입별 품질, 최근 수집 실행, 최근 추모관, 가족 권한, 제휴 문의를 확인하고 제휴 문의 상태를 `신규`, `연락 완료`, `검토 중`, `보관`으로 변경합니다. 각 제휴 문의에는 운영 담당자, 다음 연락일, 운영 메모를 저장할 수 있어 장례식장/추모시설 파트너 영업 이력을 이어받기 쉽습니다. 가족 권한은 `소유자`, `편집자`, `열람자` 역할과 `활성`, `회수` 상태를 바꿀 수 있으며, 마지막 활성 소유자를 제거하지 못하도록 서버에서 막습니다.
...
현재 프론트엔드 MVP의 `AI 태그 초안`은 로컬 규칙 기반 추천입니다. 운영 버전에서는 생애 글, 공지사항, 장면, 승인된 방명록을 LLM에 전달해 태그 후보를 만들고, 유족이 직접 눌러 승인한 태그만 공개 페이지에 반영합니다.
